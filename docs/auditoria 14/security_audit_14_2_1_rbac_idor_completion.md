# VORTIXIA - Auditoria 14.2.1: Complementação RBAC, Ownership e IDOR
...
| **COMP-IDOR-07** | Server Actions Administrativas | `NÃO APLICÁVEL` | O VORTIXIA não utiliza Server Actions no frontend. Toda comunicação mutadora ocorre via rotas REST API seguras. | ALTA | N/A | `NÃO APLICÁVEL` |
| **COMP-IDOR-08** | Endpoints Administrativos | `NÃO APLICÁVEL` | O VORTIXIA não possui rotas REST administrativas no diretório `/api/admin/*`. As rotas sob `/admin/*` são apenas páginas frontend protegidas na borda. | ALTA | N/A | `NÃO APLICÁVEL` |
