  ...
  async handleCheckout(
    ...
    try {
      ...
      const checkoutResponse = await this.provider.createCheckoutSession({
        ...
        userId: userId,
        email: user.email,
        name: user.name || "Cliente VORTIXIA",
        cpf: cpf,
        title: pkg ? `VORTIXIA - Pacote ${pkg.name}` : "Pacote de Créditos VORTIXIA",
        description: pkg?.description || "Créditos para geração de IA na plataforma VORTIXIA",
        paymentMethod,
