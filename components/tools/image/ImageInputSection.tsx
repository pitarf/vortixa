...
export function ImageInputSection({
  ...
  return (
    <div className="bg-[#0D0E12] border border-[#1E202E] rounded-2xl p-4 sm:p-5 space-y-4 shadow-xl">
      {/* Abas Superiores de Fluxo: Texto para Imagem | Imagem como Referência | Estilo de Referência */}
      <div className="grid grid-cols-3 gap-2 p-1 bg-[#070709] border border-[#1E202E] rounded-xl">
        <button
          type="button"
          onClick={() => onSelectMode("text-to-image")}
          className={`py-2 px-2 text-xs font-bold rounded-lg transition-all cursor-pointer truncate ${
        ...
        <div className="space-y-2 pt-1 border-t border-[#1E202E]/60 animate-in fade-in-50 duration-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-stretch">
            <div
              ...
              className="border-2 border-dashed border-[#1E202E] hover:border-violet-500/60 rounded-xl p-3 flex flex-col items-center justify-center text-center cursor-pointer transition-colors bg-[#070709]/60 hover:bg-[#070709]"
              style={{ minHeight: "85px" }}
            >
              {isUploadingRef ? (
                <div className="flex flex-col items-center gap-1">
                  <RefreshCw className="w-4 h-4 text-violet-400 animate-spin" />
                  <span className="text-[10px] text-slate-400">Enviando imagem...</span>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-[9px] text-slate-500">JPG, PNG, WEBP (máx. 10MB)</span>
                </div>
              )}
            </div>

            <div className="relative rounded-xl overflow-hidden border border-[#1E202E] bg-[#070709] flex flex-col items-center justify-center min-h-[85px]">
              {referenceImageUrl ? (
                <>
                  <img
                  ...
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="absolute bottom-1 px-2 py-0.5 rounded bg-black/80 text-[9px] font-bold text-white border border-white/20"
                  >
                    Trocar
                  </button>
                </>
              ) : (
                <div className="p-2 text-center text-slate-600 text-[10px]">
